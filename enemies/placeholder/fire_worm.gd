...
## FireWorm - Ranged Enemy
## Chases player and attacks from range
...
@onready var ai_controller: Node = $FireWormAI
...
func _ready() -> void:
	...
	print("[FireWorm] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[FireWorm] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
	if current_hp <= 0:
		die()
...
func die() -> void:
	print("[FireWorm] Died at %v" % global_position)
	died.emit()
