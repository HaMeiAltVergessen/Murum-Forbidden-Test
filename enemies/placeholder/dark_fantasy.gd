...
class_name DarkFantasy

## DarkFantasy - Balanced Enemy
## Well-rounded enemy with balanced stats
...
@onready var ai_controller: Node = $DarkFantasyAI
...
func _ready() -> void:
	...
	print("[DarkFantasy] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[DarkFantasy] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
	if current_hp <= 0:
		die()
...
func die() -> void:
	print("[DarkFantasy] Died at %v" % global_position)
	died.emit()
