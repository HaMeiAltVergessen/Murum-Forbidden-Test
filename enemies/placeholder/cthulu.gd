...
## Cthulu - Boss-like Enemy
## Powerful boss-tier enemy with very high health and damage
...
@onready var ai_controller: Node = $CthululuAI
...
func _ready() -> void:
	...
	print("[Cthulu] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[Cthulu] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
	if current_hp <= 0:
		die()
...
func die() -> void:
	print("[Cthulu] Died at %v" % global_position)
	died.emit()
