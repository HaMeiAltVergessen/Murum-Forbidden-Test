...
## FrostGuardian - Tank Enemy
## Defensive enemy that protects and takes heavy damage
...
const DAMAGE: int = 25
const MIN_COINS: int = 4
const MAX_COINS: int = 6
...
@onready var ai_controller: Node = $FrostGuardianAI
...
func _ready() -> void:
	...
	print("[FrostGuardian] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[FrostGuardian] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
	if current_hp <= 0:
		die()
...
func die() -> void:
	print("[FrostGuardian] Died at %v" % global_position)
	died.emit()
