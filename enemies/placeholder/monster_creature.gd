...
class_name MonsterCreature

## MonsterCreature - Medium Enemy
## Moderately fast enemy with balanced stats
...
@onready var ai_controller: Node = $MonsterCreatureAI
...
func _ready() -> void:
	...
	print("[MonsterCreature] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[MonsterCreature] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
	if current_hp <= 0:
		die()
...
func die() -> void:
	print("[MonsterCreature] Died at %v" % global_position)
	died.emit()
