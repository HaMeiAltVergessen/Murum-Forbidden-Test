...
class_name AshwormMedium

## Medium Ashworm - Burrows and lunges
...
@onready var ai_controller: Node = $AshwormMediumAI
...
func _ready() -> void:
	...
	print("[AshwormMedium] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func _on_damage_received(damage: int, knockback: Vector2, hitstun: float) -> void:
	...
	if is_burrowed:
		print("[AshwormMedium] Invulnerable while burrowed!")
		return
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[AshwormMedium] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
...
func set_burrowed(is_underground: bool) -> void:
	...
	if is_underground:
		burrowed.emit()
		print("[AshwormMedium] Burrowed (invulnerable)")
	else:
		surfaced.emit()
		print("[AshwormMedium] Surfaced (vulnerable)")
...
func die() -> void:
	print("[AshwormMedium] Died at %v" % global_position)
	died.emit()
...
func _spawn_loot() -> void:
	...
	var coin_count = randi() % 2 + 2  # 2-3 coins
