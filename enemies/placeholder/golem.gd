...
@onready var ai_controller: Node = $GolemAI
...
func _ready() -> void:
	...
	print("[Golem] Initialized at %v, HP: %d/%d" % [global_position, current_hp, MAX_HP])
...
func take_damage(amount: int, _attacker: Node = null) -> void:
	...
	print("[Golem] Took %d damage, HP: %d/%d" % [amount, current_hp, MAX_HP])
	if current_hp <= 0:
		die()
...
func die() -> void:
	print("[Golem] Died at %v" % global_position)
	died.emit()
